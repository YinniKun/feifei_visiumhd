'''
Do QC (bin level and cell level) on VisiumHD samples
Also save the QC results and segmentation results
'''
...
####### Helpers from 10X guide on binning
...
def total_umi(adata_, cut_off, path):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(path, "bin_total_umi_distribution.png"))
    plt.close()
...
def qc(space_ranger, output) -> None:
    ...
    # adata.write(f"{output_file}_processed.h5ad")

def segment(img: np.ndarray, dir_base: str, model_name: StarDist2D = "2D_versatile_he", 
            normalize_img: bool = True) -> gpd.GeoDataFrame:
    ...
    labels, polys = model.predict_instances_big(img, axes='YXC', 
                                                block_size=4096, prob_thresh=0.01,nms_thresh=0.001, 
...
def binning(base_path: str, gdf: gpd.GeoDataFrame, output: str) -> None:
    ...
    filtered_adata.obs =  pd.merge(filtered_adata.obs, barcodes_in_one_polygon[['index','geometry','id','is_within_polygon','is_not_in_an_polygon_overlap']], 
    ...
    grouped_filtered_adata = anndata.AnnData(X=summed_counts,
                                             obs=pd.DataFrame(polygon_id,columns=['id'],index=polygon_id),var=filtered_adata.var)
...
def main():
    ...
    parser.add_argument("--space_ranger", help="Path to the input SpaceRanger directory")
    parser.add_argument("--tissue_img", help="Path to the input tissue image file")
    parser.add_argument("--output", help="Path to the output file, summarizing QC metrics")
